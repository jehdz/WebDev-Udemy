...
//finally --> this is a new feature
//it allows us to do something asfter the promise has finally has finished
//aftereverything is done, finally will be called weather its resolved or rejected
const urls = [
...
Promise.all(urls.map(url => fetch(url).then(people => people.json())))
    ...
    //finally() is good when you need to run a piece of code no matter what
...
//using the for of loop, we are now able to iterate through the await promisese
